...
const add = () => {
  ...
  request.post('lost/add',data.form).then(res => {
    if (res.code === '200'){
...
const update = () => {
  request.put('lost/update',data.form).then(res => {
    if (res.code === '200'){
      ElMessage.success('操作成功')
      load()
    }else{
...
const del = (id) =>{
  ElMessageBox.confirm('删除数据后无法恢复，是否确认删除？','删除确认',{type:'warning'}).then(() =>{
    request.delete(`lost/delete?id=${id}`).then(res =>{
      if (res.code === '200'){
        ElMessage.success('操作成功')
        load()
      }else{
...
const delBatch = () =>{
  ...
  ElMessageBox.confirm('删除数据后无法恢复，是否确认删除？','删除确认',{type:'warning'}).then(() => {
    request.delete('lost/deleteBatch',{data: data.ids}).then(res =>{
      if (res.code === '200'){
        ElMessage.success('操作成功')
        load()
      }else{
